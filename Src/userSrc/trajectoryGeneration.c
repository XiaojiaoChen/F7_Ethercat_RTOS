 ...
 */
static void StepInit(TYPE_STEP *s,float x0,float x1,float T,float *x_out,float *v_out,float *a_out);
static void SquareInit(TYPE_SQUARE *s,float x0,float x1,float T,float *x_out,float *v_out,float *a_out);
static void StepScurveInit(TYPE_STEPSCURVE *s,float x0,float x1,float T,float *x_out,float *v_out,float *a_out);
static void SquareScurveInit(TYPE_SQUARESCURVE *s,float x0,float x1,float transT,float *x_out,float *v_out,float *a_out);
static void SinoidInit(TYPE_SINOID *s,float x0,float v0,float x1,float T,float *x_out,float *v_out,float *a_out);
...
{
	float *x_out=traj->x_out;
	float *v_out=traj->v_out;
	float *a_out=traj->a_out;
		...
		if(lineType == TRAJ_GEN_STEP_SCURVE)
		{
			...
			StepScurveInit(&(traj->stepScurve),traj->x0,xd,duration/1000.0f,x_out,v_out,a_out);
		}
		else if(lineType == TRAJ_GEN_STEP)
		{
			traj->tDuration = duration;
			traj->generator = StepGenerate;
			StepInit(&(traj->step),traj->x0,xd,duration/1000.0f,x_out,v_out,a_out);
		...
		else if(lineType == TRAJ_GEN_SQUARE)
		{
			...
			SquareInit(&(traj->square),traj->x0,xd,duration/1000.0f,x_out,v_out,a_out);
		...
		else if(lineType == TRAJ_GEN_SQUARE_SCURVE)
		{
			...
			SquareScurveInit(&(traj->squareScurve),traj->x0,xd,duration/1000.0f,x_out,v_out,a_out);
		...
		else if(lineType == TRAJ_GEN_SINOID)
		{
			...
			SinoidInit(&(traj->sinoid),traj->x0,traj->v0,xd,duration/1000.0f*4,x_out,v_out,a_out);
		}
...
static void SquareScurveInit(TYPE_SQUARESCURVE *s,float x0,float x1,float transT,float *x_out,float *v_out,float *a_out)
{
	...
	StepScurveInit(&(s->scurve[0]),x0,x1,transT,x_out,v_out,a_out);
	StepScurveInit(&(s->scurve[1]),x1,-x1,transT,x_out,v_out,a_out);
	StepScurveInit(&(s->scurve[2]),-x1,x1,transT,x_out,v_out,a_out);
	s->x0 = x0;
	s->x1 = x1;
	s->x_out = x_out;
	s->v_out = v_out;
...
static void SinoidInit(TYPE_SINOID *s,float x0,float v0,float amp,float periodQuater,float *x_out,float *v_out,float *a_out)
{
	float phival = x0/amp;
	s->x0 = x0;
	s->v0 = v0;
